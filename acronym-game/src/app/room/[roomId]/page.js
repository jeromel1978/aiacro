...
export default function GameRoomPage() {
  ...
  const mockIsCurrentUserTheOldestForUITesting = !session && players.length > 0 && players[0].name === 'Alice' && players[0].isOldest;
  ...
  useEffect(() => {
    ...
    if (gamePhase === 'PhraseEntry') {
      ...
      const mockPhrases = [
        ...
        { id: 'phrase3', text: `${currentAcronym[0]} Really ${currentAcronym[1]} Good ${currentAcronym[2]}${currentAcronym[3] ? ' '+currentAcronym[3] : ''} Submission`, acronym: currentAcronym.join('') },
    ...
    return cleanupTimer; 
  }, [gamePhase]); // currentRoundNumber is not a direct dependency here, it's set when 'PhraseEntry' starts
      ...
      <div className="lg:w-2/3 flex flex-col gap-4">
        ...
        <Card className="flex-grow">
          ...
          <CardContent className="flex flex-col items-center justify-center h-full p-2 sm:p-4">
            ...
            {gamePhase === 'Voting' && (
              <div className="w-full">
                ...
                <div className="space-y-3 overflow-y-auto max-h-[calc(100vh-var(--header-height,80px)-250px)] p-1">
                  {submittedPhrases.length > 0 ? submittedPhrases.map((phrase) => (
                    <Card 
                      key={phrase.id} 
                      className={`p-3 flex justify-between items-center transition-all cursor-pointer
                                  ${playerVote === phrase.id ? 'border-blue-500 ring-2 ring-blue-500 dark:border-blue-400' : 'border-gray-200 dark:border-gray-700 hover:shadow-md'}
                    ...
                    >
                      ...
                      <Button
                        onClick={(e) => { e.stopPropagation(); handleVote(phrase.id); }} // Prevent card click from double-triggering if button is inside
            ...
            {gamePhase === 'Results' && (
              <p className="text-lg">Round results will be shown here...</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="flex-grow flex flex-col">
          ...
          {gamePhase === 'PhraseEntry' && (
            <>
              <CardContent className="flex-grow flex flex-col justify-center p-4">
                {hasSubmittedPhrase ? (
                  <p className="text-center text-gray-600 dark:text-gray-400">Your phrase is submitted. Waiting for others...</p>
                ) : timerValue === 0 ? (
                  ...
                  <Textarea
                    ...
                    className={`resize-none flex-grow text-base ${!isPhraseValid && phraseInputValue ? 'border-red-500 dark:border-red-500 focus-visible:ring-red-500' : ''}`}
                    rows={5}
          ...
          {gamePhase === 'Voting' && (
            <CardContent className="flex-grow p-4">
              ...
              <p className="mt-2 text-xs text-gray-500 dark:text-gray-500">You can only vote once. Voting ends when the timer runs out.</p>
              {playerVote && (
